...
- **Python**: 3.10+
- **Web сервер**: Nginx с SSL сертификатами
- **База данных**: PostgreSQL
- **Менеджер процессов**: Gunicorn/uWSGI
...
#### 🔴 AI API запросы к нейросетям
- **Проблема**: Подключение к API нейросетей работает нестабильно
- **Симптомы**: 
  - Ошибки подключения к OpenAI API
  - Таймауты при обращении к Anthropic Claude
  - Некорректные ответы от Perplexity API
- **Влияние**: AI ассистент может работать неправильно или не работать вообще
- **Статус**: 🔧 Требует исправления
- **Приоритет**: Высокий

#### 🟡 Другие известные проблемы
- Возможны проблемы с авторизацией через Telegram WebApp
- Иногда происходят сбои при сохранении задач в базе данных
- Frontend может отображать устаревшие данные при плохом соединении

### Для участия в исправлении:
1. Проверьте секцию [Issues](https://github.com/AndreyTsibin/tudushka/issues)
